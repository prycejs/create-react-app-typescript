...
## SASS support for react-scripts-1.x
...
1. `react-scripts-ts` is a fork of `react-scripts 1.x` which is part of `create-react-app 1.x`
2. `react-scripts-ts` adds typescript support to `react-scripts`
3. `react-scripts 1.x` does not have SASS support currently, but will handle SASS natively in `create-react-app 2.x`
4. `create-react-app 2.x` has been released with `react-scripts` update that has native SASS support
5. `react-scripts-ts` has not yet integrated the changes from `create-react-app 2.x` (but will soon).
